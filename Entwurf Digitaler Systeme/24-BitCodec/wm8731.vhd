...
port (clk, reset : in std_logic; -- Takteingang (entsprechend 18,432 MHz fuer WM8731)
	  din : in std_logic_vector(31 downto 0); -- Audiodaten: 16 Bit linker Kanal + 16 Bit rechter Kanal (dds rechtecksignal)
...
constant i2c_data : rom := (
  ...
  "0001000000000010", -- R8  Normal Modus, 384fs (BOSR=1) (MCLK/XCK = 18,432 Mhz, f_s = 48 kHz)
  "0001001000000000", -- R9  Deaktiviere Audio
  "0001001000000001"  -- R9  Aktiviere Audio
);

signal s_din : std_logic_vector(31 downto 0); -- 2x16bit fuer left und right channel
signal s_lrswitch : integer range 0 to 2303; -- wieso 2303?
...
begin
    ...
    -- Zaehlung von ROM Data, 32 Bits pro Wort, Sequenz von jeweils 3 Bit
		-- i2c_seq zaehlt jeweils von 0 bis 2
		-- i2c_bitcnt zaehlt jeweils von 0 bis 31 (Laenge einer i2c message)
		-- i2c_wordcnt wird nach 32 Schritte von i2c_bitcnt inkrementiert
	    	-- wenn i2c_wordcnt auf 7 steht, ist das Schreiben ueber I2C beendet
	...
	case i2c_bitcnt is
		when 0 => i2c_sdat <= '1'; -- high
		when 1 => i2c_sdat <= '0'; -- low Startbedingung
		when 2 => i2c_sdat <= i2c_adr(6);-- 0 7 Bit I2C Adresse
		when 3 => i2c_sdat <= i2c_adr(5);-- 0
		when 4 => i2c_sdat <= i2c_adr(4);-- 1
		when 5 => i2c_sdat <= i2c_adr(3);-- 1
		when 6 => i2c_sdat <= i2c_adr(2);-- 0
		when 7 => i2c_sdat <= i2c_adr(1);-- 1
		when 8 => i2c_sdat <= i2c_adr(0);-- 0
		when 9 => i2c_sdat <= '0'; -- '0' fuer Write Zugriff 
		-- 1 bit Acknowledge
		when 10 => i2c_sdat <= 'Z'; -- release the sda line
		...
		-- 1 bit Acknowledge
		when 19 => i2c_sdat <= 'Z'; -- release the sda line
		...
		-- 1 bit Acknowledge
		when 28 => i2c_sdat <= 'Z'; -- release the sda line
		when 29 => i2c_sdat <= '0'; -- low Stopbedingung
		when others  => i2c_sdat <= '1'; -- high Stoppbedingung
    	end case;
	end if;
	end if;
end process;
...
begin
	...
	if s_lrswitch = 383 then
    	...
    	-- if s_lrswitch = 383 then -- 18,432 Mhz / 384 = 48 kHz (siehe Datenblatt Seite 42)
  		-- aud_daclrck <= '1'; -- Gib Codec Bescheid, dass Daten hier anfangen (nur ein impuls bei dsp mode a)
  		-- s_din <= din; -- 16 bit pro channel
		-- s_lrswitch <= 0; -- reset des s_lrswitch von 383 zu 0
    	-- else
	  	-- aud_daclrck <= '0'; -- bleibe sonst null
	  	-- s_din <= s_din(30 downto 0) & "0"; -- zum shiften
	  	-- s_lrswitch <=  s_lrswitch + 1; -- increment s_lrswitch
    	-- end if;
  	end if;
end process;

i2c_wrdcnt <= std_logic_vector(to_unsigned(i2c_wordcnt, 4));
aud_xck  <= clk; -- gebe mclk/xck vor fuer codec
aud_bclk <= not clk; -- invertiere fuer dsp mode A fuer ein tak spaeter zu aud_daclrck
aud_dacdat <= s_din(31); -- fuettere codec serieller dateninput
end behavioral;