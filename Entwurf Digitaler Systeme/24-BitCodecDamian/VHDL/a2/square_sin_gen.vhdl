...
architecture behavioral of square_sin_gen is
    ...
    signal max_count_square : integer range 0 to 8807 := 0; -- max valaue ist 8807;  -- Note C6 (1046,5 Hz)
    signal max_count_dds : integer range 0 to 587  := 0; -- max wert: C6
   
    -- Interner Taktz�hler
    signal tone_cnt_square : integer range 0 to 8807 := 0;
    signal tone_cnt_dds : integer range 0 to 587  := 0;
    
    signal square_sig : std_logic := '0'; -- Zustand Rechtecksignals 0 oder 1
    signal dds_en : std_logic := '0'; -- eingang f�r sin dds
...
begin
    ...
    begin
        ...
        -- 
            ...
            if max_count_square = 0 then -- kein key gedr�ckt
                tone_cnt_square   <= 0;
                square_sig <= '0';
            else
                -- Frequenzz�hler l�uft
                if tone_cnt_square >= (max_count_square - 1) then -- max wert erreicht? -1 da bei 0 anf�ngt zu z�hlen
                    tone_cnt_square <= 0;
    ...
    square_sample <= AMP_HIGH when (max_count_square /= 0 and square_sig = '1') else -- Key gedr�ckt, Osz: H
                      AMP_LOW  when (max_count_square /= 0 and square_sig = '0') else -- Key gedr�ckt, Osz: L
    ...
    process(rst, aud_xck)
       begin
            if rst = '1' then -- rst aktiv
                tone_cnt_dds <= 0;
                dds_en <= '0';

            elsif rising_edge(aud_xck) then
                dds_en <= '0';
                if max_count_dds /= 0 then -- knopf gedr�ckt
                    if tone_cnt_dds >= (max_count_dds - 1) then
                        tone_cnt_dds <= 0;
                        dds_en <= '1'; -- in dds clock -> n�chster wert
                    else
                        tone_cnt_dds <= tone_cnt_dds + 1;
                    end if;
                else
                    tone_cnt_dds <= 0;
                end if;
    ...
    dds_inst : my_dds
        port map (
            clk => dds_en,
            rst => rst,
            audio_out => dds_sample
        );



    -- AusgangsMUX
    process(sw_mode, square_sample, dds_sample, max_count_dds)
        begin
        if max_count_dds = 0 then -- nichts es kann genau so gut max_count_square gepr�ft werden
            current_sample <= AMP_ZERO;

        elsif sw_mode = '1' then -- sinus 
            current_sample <= dds_sample;

        else -- rechteck
            current_sample <= square_sample;
        end if;
    end process;

    audio_out <= current_sample & current_sample; -- &: verkettungsoperator
    -- audio_out <= dds_sample & dds_sample;
