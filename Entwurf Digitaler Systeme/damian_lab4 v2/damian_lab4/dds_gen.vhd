...
signal akkumulator : unsigned(addr_width+10-1 downto 0);
...
process(clk, reset)
    begin
        if reset = '1' then
            ...
            akkumulator <= akkumulator + to_unsigned(einstellwert, akkumulator'length);
        end if;
end process;
...
PORT MAP(
addrin => std_logic_vector(akkumulator(addr_width+10-1 downto 10)), 
dataout => dds_data
);
